	...
	$conditionsChecked = !empty($this->request->data['condition']) || !$this->Configuration->getKey('condition');
            if (!empty($this->request->data['pseudo']) && !empty($this->request->data['password']) && $conditionsChecked && !empty($this->request->data['password_confirmation']) && !empty($this->request->data['email'])) { // si tout les champs sont bien remplis
                ...
                if (strlen($this->request->data['password']) < 6){
                    $this->response->body(json_encode(array('statut' => false, 'msg' => "Votre mot de passe doit être composé d'au moins 6 carractères !")));
                } else {
                    ...
                    if ($validCaptcha) { // on check le captcha déjà
                        ...
                        if ($isValid === true) { // on vérifie si y'a aucune erreur
                            ...
                            copy('../webroot/textures/defaults/cape.png', '../webroot/textures/capes/' . $this->request->data['pseudo'] . '.png');
                            copy('../webroot/textures/defaults/skin.png', '../webroot/textures/skins/' . $this->request->data['pseudo'] . '.png');
        ...
        if ($this->isConnected && $this->Permissions->can('MANAGE_USERS')) {
            if ($this->request->is('post')) {
                ...
                if (!empty($this->request->data['id']) && !empty($this->request->data['email']) && !empty($this->request->data['pseudo']) && (!empty($this->request->data['rank']) || $this->request->data['rank'] == 0)) {
                    ...
                    rename('../webroot/textures/capes/' . $findUser['User']['pseudo'] . '.png', '../webroot/textures/capes/' . $this->request->data['pseudo'] . '.png');
                    rename('../webroot/textures/skins/' . $findUser['User']['pseudo'] . '.png', '../webroot/textures/skins/' . $this->request->data['pseudo'] . '.png');
        ...
        if ($this->isConnected AND $this->Permissions->can('MANAGE_USERS')) {
            if ($id != false) {
                ...
                if (!empty($find)) {
                    ...
                    $username = $this->User->find('first', array('conditions' => array('id' => intval($id))))['User']['pseudo'];
                    unlink(ROOT.DS.'app'.DS.'webroot/textures/skins/' . $username . '.png');
                    unlink(ROOT.DS.'app'.DS.'webroot/textures/capes/' . $username . '.png');
